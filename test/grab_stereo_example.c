...
int main(int argc, char *argv[])
{
    ...
    uint8_t *stereoData = (uint8_t *)malloc(width*height*2);
    err=dc1394_deinterlace_stereo(frame->image,stereoData,width,height*2);

    // colorize crap
    uint8_t *rgb = (uint8_t *)malloc(width*height*2*3);
    err=dc1394_bayer_decoding_8bit(stereoData,rgb,width,height*2,DC1394_COLOR_FILTER_BGGR,DC1394_BAYER_METHOD_SIMPLE);
    ...
    fwrite(stereoData, 1, height * width*2, imagefile);
    fclose(imagefile);
    printf("wrote: Part.pgm\n");

    imagefile=fopen("Part.ppm","wb");
    fprintf(imagefile,"P6\n%u %u\n255\n", width, height*2);
    fwrite(rgb, 1, 2*height*width*3, imagefile);
    fclose(imagefile);
